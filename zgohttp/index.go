...
type Httper interface {
	JsonpOK(ctx iris.Context, r interface{}) error
	JsonpErr(ctx iris.Context, msg string) error
	JsonOK(ctx iris.Context, r interface{}) error
	JsonServiceErr(ctx iris.Context) error
	JsonParamErr(ctx iris.Context) error
	JsonErr(ctx iris.Context, status int, code string, msg string) error
	JsonExpectErr(ctx iris.Context, msg string) error
	JsonFree(ctx iris.Context, content interface{}) error // 自定义返回结构体
...
func (zh *zgohttp) JsonpOK(ctx iris.Context, r interface{}) error {
...
func (zh *zgohttp) JsonpErr(ctx iris.Context, msg string) error {
	startTime := ctx.Values().GetInt64Default("startTime", time.Now().UnixNano())
	takeTime := (time.Now().UnixNano() - startTime) / 1e6
...
// JsonOK 正常的返回方法
func (zh *zgohttp) JsonOK(ctx iris.Context, r interface{}) error {
	startTime := ctx.Values().GetInt64Default("startTime", time.Now().UnixNano())
	takeTime := (time.Now().UnixNano() - startTime) / 1e6
...
// JsonOK 正常的返回方法
func (zh *zgohttp) JsonFree(ctx iris.Context, content interface{}) error {
	//startTime := ctx.Values().GetInt64Default("startTime", time.Now().UnixNano())
	//takeTime := (time.Now().UnixNano() - startTime) / 1e6
...
func (zh *zgohttp) JsonExpectErr(ctx iris.Context, msg string) error {
	startTime := ctx.Values().GetInt64Default("startTime", time.Now().UnixNano())
	takeTime := (time.Now().UnixNano() - startTime) / 1e6
...
func (zh *zgohttp) JsonErr(ctx iris.Context, status int, code string, msg string) error {
	startTime := ctx.Values().GetInt64Default("startTime", time.Now().UnixNano())
	takeTime := (time.Now().UnixNano() - startTime) / 1e6
...
func (zh *zgohttp) JsonServiceErr(ctx iris.Context) error {
	msg := "服务器开小差了，稍后再试吧"
	startTime := ctx.Values().GetInt64Default("startTime", time.Now().UnixNano())
	takeTime := (time.Now().UnixNano() - startTime) / 1e6
	return ctx.JSON(ErrResponse{Status: 500, Msg: msg, ErrorCode: "500", Data: make(map[string]interface{}), Time: takeTime})
...
func (zh *zgohttp) JsonParamErr(ctx iris.Context) error {
	msg := "参数错误"
	startTime := ctx.Values().GetInt64Default("startTime", time.Now().UnixNano())
	takeTime := (time.Now().UnixNano() - startTime) / 1e6
...
func (zh *zgohttp) UseBefore(ctx iris.Context) {
	defer func() {
		if err := recover(); err != nil {
			...
			logMessage += fmt.Sprintf("At Request: %s\n", getRequestLogs(context.Context{}))
